...
namespace THA_W9_CONNECTSQL
{
    public partial class Form1 : Form
    {
        ...
        private void comboboxplayer_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            sqlQuery = $"SELECT p.player_name as 'Name', t.team_name as 'Team Name' , p.playing_pos as 'Playing Pos' , n.nation as 'Nationality', ifnull(sum(dm.type='CY'),0) as 'Yellow Cards', ifnull(sum(dm.type='CR') , 0) as 'Red Cards',ifnull(sum(dm.type='GO') , 0) as 'Goal Scored', ifnull(sum(dm.type='PM') , 0 ) as 'Penalty Missed' FROM player p left JOIN team t on t.team_id = p.team_id left join dmatch dm on dm.player_id = p.player_id and dm.team_id = t.team_id inner join nationality n on n.nationality_id = p.nationality_id where p.player_id = '{a}' order by p.player_name asc;";
            cmd = new MySqlCommand(sqlQuery, conn);
            adapter = new MySqlDataAdapter(cmd);
            adapter.Fill(dtpos);
        ...
        private void comboboxmatch_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            dtplayer2.Clear();
            //sqlQuery = $"select * from dmatch where match_id = '{comboboxmatch.SelectedValue.ToString()}';";
            sqlQuery = $"select p.player_name as Team_Home from player p, db_theo.match m where m.match_id = '{comboboxmatch.SelectedValue.ToString()}' and p.team_id = m.team_home;";
            cmd = new MySqlCommand(sqlQuery, conn);
            adapter = new MySqlDataAdapter(cmd);
            adapter.Fill(dtplayer2);
            dgvhome.DataSource = dtplayer2;
            dtplayer3.Clear();
            //sqlQuery = $"select * from dmatch where match_id = '{comboboxmatch.SelectedValue.ToString()}';";
            sqlQuery = $"select p.player_name as Team_Away from player p, db_theo.match m where m.match_id = '{comboboxmatch.SelectedValue.ToString()}' and p.team_id = m.team_away;";
            cmd = new MySqlCommand(sqlQuery, conn);
            adapter = new MySqlDataAdapter(cmd);
            adapter.Fill(dtplayer3);
            dgvaway.DataSource = dtplayer3;
        }
        ...
        private void dgvaway_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
